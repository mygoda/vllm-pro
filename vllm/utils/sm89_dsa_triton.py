...
fp32 matmul (``input_precision="ieee"``) to match the torch reference exactly.
The 4090 has no fp32 tensor cores, so the win is memory traffic, not flops; if
the reference tolerance can be relaxed to topk-overlap, casting the dot inputs
to bf16 unlocks the Ada bf16 tensor cores — that's the upgrade path.
"""
...
def triton_mqa_logits(
    ...
    out = q_f.new_empty((M, N))

    BLOCK_M, BLOCK_N = 64, 128
    BLOCK_D = triton.next_power_of_2(D)
    grid = (triton.cdiv(M, BLOCK_M), triton.cdiv(N, BLOCK_N))
    _mqa_logits_kernel[grid](
        q_f, k_f, weights, ks, ke, out,
        M, N, H,
